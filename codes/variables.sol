...
contract MyContract{ // The first line is declaring a state variable
    ...
    // we'll be using 'view' since we are not making changes to the state variabble and we are only declaring a local variable and setting its value to a local variable
